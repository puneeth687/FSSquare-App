...
function InventoryScreen({route, navigation}) {
    ...
    const vfUrl = `${sfInstanceUrl}/apex/InventoryVF`;
    const handleLoadEnd = (syntheticEvent) => {
        const url = syntheticEvent.nativeEvent.url;
        console.log("Loaded URL:", url);

        if (step === 1) {
            console.log("Frontdoor done, navigating to VF...");
            setStep(2);
            setLoading(true); // Keep loader ON while VF loads
        } else {
            setLoading(false); // Only hide loader when VF is done
        }
    };
